...
// Convert C++ content to LearnLanguage format
export const getCppLanguageContent = (): LearnLanguage => {
  return {
    id: "cpp",
    name: "C++",
    icon: "Binary",
    topics: topicsOfCpp.map((topic) => ({
      id: `cpp-${topic.toLowerCase().replace(/\s+/g, "-")}`,
      title: topic,
      content: topicContentsOfCpp[topic]?.content || "",
      code: topicContentsOfCpp[topic]?.code,
    })),
  };
};
