...
export const topicContentsOfHtml: { [key: string]: { title: string; content: string; code?: string } } = {
    "HTML Introduction": {
...
// Convert HTML content to LearnLanguage format
export const getHtmlLanguageContent = (): LearnLanguage => {
  return {
    id: "html",
    name: "HTML",
    icon: "Code",
    topics: topicsOfHtml.map((topic) => ({
      id: `html-${topic.toLowerCase().replace(/\s+/g, "-")}`,
      title: topic,
      content: topicContentsOfHtml[topic]?.content || "",
      code: topicContentsOfHtml[topic]?.code,
    })),
  };
};